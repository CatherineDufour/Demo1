    ...
    const VerifyPresentation = (presentation_exchange_id) => {
        fetch('/present-proof/records/' + presentation_exchange_id + '/verify-presentation', 
         ...
         .then(data => {
            ...
            props.history.push('/preuveinscription', {
                connection_id              : props.location.state.connection_id,
                data : {
                    succursale             : props.location.state.data.succursale, 
                    classe                 : props.location.state.data.classe, 
                    periode                : props.location.state.data.periode, 
                    holderId               : data.presentation.requested_proof.revealed_attrs.holderId.raw, 
                    holderType             : data.presentation.requested_proof.revealed_attrs.holderType.raw, 
